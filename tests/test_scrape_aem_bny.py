"""
tests/test_scrape_aem_bny.py

Covers scrape_aem_bny.parse_listing_page() -- the listing-page card parser
for BNY's press-release site -- plus date_from_url()'s URL-embedded-date
fallback.
...
scrape_aem_bny.py's module docstring for why they're now separate scraper
...
capture:
    ...
    python src/scrape_aem_bny.py --dry-run --year 2026 --slug bny \
...
from scrape_aem_bny import date_from_url, parse_listing_page  # noqa: E402
...
def test_date_from_url_handles_zero_padded_path_segments():
    url = "https://example.com/content/foo/2026/01/15/some-title.html"
    assert date_from_url(url) == date(2026, 1, 15)


def test_date_from_url_handles_hyphenated_slug_dates():
